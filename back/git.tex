...
\begin{enumerate}
  ...
  \item create a virtual environment using \py{virtualenv venv}
  \item activate virtual environment using \py{venv\scripts\activate}
  \item install dependencies \py{pip install -r requirements.txt}
\end{enumerate}
...
\begin{enumerate}
  \item activate virtual environment using \py{venv\scripts\activate}
  \item start project server using \py{python live.py}
  \item open browser on 127.0.0.1:5000
  \item enjoy the site
  \item stop project server by pressing \py{Ctrl + c}
  \item deactivate virtual env using \py{deactivate}
\end{enumerate}

You can also start a Gunicorn production server with this command: \py{gunicorn -c guni.py wsgi:app}, which starts the site on 127.0.0.1:8000.
