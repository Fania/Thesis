...
\paragraph{Grammars}
We could create a whole new language grammar based on pataphysical principles. Examples of using a standard grammar (see chapter~\ref{s:grammars}\marginnote{§~\ref{s:grammars}}) for generating `random' text are as follows.
...
In chapter~\ref{s:csf}\marginnote{§~\ref{s:csf}} I discussed the concepts of uninspiration and aberration by Wiggins and Ritchie \citeyear{Wiggins2006,Ritchie2012}. 
...
\todo{if pataphysicalisation happens at different locations... }
They are all possible and will maybe be done all at the same time\marginnote{\faicon{object-group}~\ref{fig:patasearch02f}}. We can consider the possibility of a `patametric index', rather than a parametric index or a `patasaurus' (pataphysical thesaurus/ontology).
...
\end{figure}
\todo{copy and adapt figure }
\begin{figure}[!htbp]
  \centering
  \begin{tikzpicture}[node distance = 1.5cm and 2cm]
    \spiral[]{center={(4,-2.3)}, end angle=45, end radius= 0.3, revolutions=4};
