...
\ntodo[inline]{Sort out Table of Equations 1}
...
\itab{$R\left ( q_{i}, d_{j} \right )$}
\tab{the ranking function, where $q_{i} \in Q$ and $d_{j} \in D$,}\\
\itab{$t$} \tab{the number of index terms in a document collection,}\\
\itab{$V =\left \{k_{1},\ldots, k_{t}\right \}$}
\tab{is the set of all distinct index terms in a document collection (vocabulary).}

The term conjunctive component describes which terms occur ina  document and which ones do not. E.g. for $V =\left \{k_{1},\ldots, k_{t}\right \}$, if $\left [ k_{1},k_{2},k_{3} \right ]$ occur in $d_{j}$ then it could be $\left ( 1,1,1 \right )$ which means all terms occur, or $\left ( 1,0,0 \right )$ which means only term $k_{1}$ appears in $d_{j}$.

\itab{$c\left ( d \right )$} \tab{is the term conjunctive component for document $d$}\\
\itab{$c\left ( q \right )$} \tab{is the term conjunctive component for query $q$}
...
\centering
\includegraphics[width=4cm]{PageRank}
...
Where,\\
\itab{$L\left(p\right)$} \tab{is the number of outgoing links of page $p$}\\
...
Manning, C., Raghavan, P. and Schuetze, H., 2008. Introduction to Information Retrieval 1st ed., Cambridge: Cambridge University Press.\citep{Manning2008}
