...
Studying Jarry's life gives certain insights into the man who created pataphysics and why he might have done so. Several works have helped prepare the below outline of Jarry's life. Alastair Brotchie's \textit{A Pataphysical Life} \citeyear{Brotchie2011} and Roger Shattuck's \textit{The Banquet Years} \citeyear{Shattuck1959} were the two main sources used but several others have also written about Alfred Jarry (e.g. Linda Klieger Stillman, Keith Beaumont, and Jill Fell).
...
Techniques such as the famous `N+7', `melting snowball' and `Mathew\'s Algorithm' (see chapter~\ref{s:oulipo} and figure~\ref{fig:poems}\marginnote{\faicon{object-group}~\ref{fig:poems}}) are typical examples of oulipian methods. They have endless applications in as many different disciplines. The use of constraints is now a well-known approach for creative activities and has many supporters while still being aware of counter-arguments. For example, Motte explained that ``Erecting the aesthetic of formal constraint, then, the Oulipo simultaneously devalues inspiration'' \autocite[p.10]{Motte2007}.

François Le Lionnais defines three levels in the hierarchy of constraints \autocite[p.11]{Motte2007}.

\begin{description}
  \item[Minimal] constraints on the language in which the text is written
  \item[Intermediate] constraints on genre and certain literary norms
  \item[Maximum] consciously preelaborated and voluntarily imposed systems of artifice
\end{description}

So, constraints are often based on the alphabet or mathematical in nature.

% Please add the following required packages to your document preamble:
% \usepackage{booktabs}
\begin{table}[!htbp]
\centering
\caption{Elementary Linguistic and Literary Operations}
\label{tab:oulipo}
\begin{tabu}{lllllllll}
  \toprule
  & \textbf{Displacement} & \textbf{Substitution} & \textbf{Addition} & \textbf{Subtraction} & \textbf{Multiplication (repetition)} & \textbf{Division} & \textbf{Deduction} & \textbf{Contraction}
  \\ 
  \midrule
  \textbf{Letter}    
  &
...
\todo{talk about the role of chance - aleatoricism}

\begin{quotation}
  The Oulipo is anti-chance. \sourceatright{Claude Berge in \autocite[p.17]{Motte2007}}
\end{quotation}

\todo{look up quote oulipo and aleatoricism}
...
The influence of Jose Luis Borges was already briefly discussed in chapter~\ref{s:borges}\marginnote{§~\ref{s:borges}}. Hugill sees him as an unconscious pataphysician \autocite{Hugill2012a}.
...
\begin{quotation}
  ...
  Likewise, string theory and quantum calculations are, increasingly, not descriptive of an actual reality, but are simply mathematical pataphors.\footnote{\url{http://www.urbandictionary.com/define.php?term=pataphysics}}
\end{quotation}
