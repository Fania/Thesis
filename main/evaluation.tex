...
Others argue that creativity can be measured by looking at the overall `unexpectedness' of an artefact \autocite{Kazjon2014}. 
...
\section{From the Evaluation to Paris by Sea}
...
\vspace*{-0.3cm}
The chapter starts with an analysis of how search is evaluated in computers. This relates to the topics of search and ranking discussed in the \nameref{ch:technology} \tech~ chapter. The discourse on evaluating creative computers very much overlaps with the \nameref{ch:creativity} \creat~ chapter and the comparisons drawn in chapter~\ref{ch:foundations} \found. Specific evaluation models such as the \ac{TMPR} \autocite{Edmonds2010} and \ac{MMCE} \autocite{Candy2012} are mentioned in chapter~\ref{ch:methodology} \metho. Jordanous' \ac{SPECS} model is detailed in chapter~\ref{ch:interpretation} \inter~ as a basis for the original contribution that is the evaluation framework in section~\ref{s:framework} \inter. This chapter also contains a discussion of the \ac{CSF} by Wiggins \autocite*{Wiggins2006} \inter~ and a proposal for an alternate pataphysical version of the \ac{CSF} is given in chapter~\ref{ch:future} \aspi.
