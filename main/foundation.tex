...
\begin{description}
  \item [4 P Model] Mel Rhodes identified four common themes of creativity (Person, Process, Press, Products), which he termed the `4 P\rq s' of creativity \citeyear{Rhodes1961}.\marginnote{§~\ref{s:fourp}}
  \item [4 Aspects] Ross Mooney independently identified four aspects of creativity which he called Environment, Person, Process and Product \autocite[as cited in][]{Sternberg1999}.\marginnote{§~\ref{s:fourp}}
  \item [P and H Model] Margaret Boden defined three types of creativity: combinational, exploratory and transformational and two different `levels' P and H creativity \citeyear{Boden2003}.\marginnote{§~\ref{s:boden}}
  \item [4 C Model] James Kaufman and Ronald Beghetto defined the `4 C' model of creativity. These are Big-C, Pro-c, Little-c and Mini-c \citeyear{Kaufman2009}.\marginnote{§~\ref{s:fourc}}
\end{description}
...
\begin{description}
  \item [4 Stage Model] Henri Poincaré suggested a `4 Stage Model' (formulated by Graham Wallas in 1926). The stages are: preparation, incubation, illumination and verification \citeyear{Poincare2001, Wallas1926}.\marginnote{§~\ref{s:4stages}}
  \item [Problem Solving] George Pólya came up with a description of the `problem solving' process \citeyear{Polya1957}.\marginnote{§~\ref{s:4stages}}
\end{description}
...
\begin{table}[!htbp]
\centering
\caption[Creation vs engineering vs abstraction]{Comparison of artistic creation, software engineering and abstraction by \autocite{Hugill2013c}}
\label{tab:acsea}
\begin{tabu}{lll}
\toprule
...
Table~\ref{tab:ccdhcc}\marginnote{\faicon{table}~\ref{tab:ccdhcc}} shows the four steps of creative computing defined by Andrew Hugill and Hongji Yang \autocite{Hugill2013c} and lines them up with corresponding activities in \ac{DH} \autocite{Burdick2012}, \ac{CompC} \autocite{Colton2012} and Computer Ethics \autocite{Stahl2013}.
...
\begin{draft}
  ...
  \end{itemize}
\end{draft}
\todo{could thse opositions be expressed as antonomies?}
...
Given a query term X, what is considered a relevant match though? Do we simply return a list of Web pages where X appears in the heading of each page? It is obviously not that easy. Several ranking signals are combined together; Google states that they use over \num{200} signals including PageRank and they personalise results using signals such as the web history and location (Google n.d.).
...
\todo{It weould be nice to conclude these thoughts with some more positive suggestions about ways to do this}
