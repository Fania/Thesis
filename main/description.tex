...
The website \url{http://pata.physics.wtf} showcases the current proof-of-concept algorithms. This chapter gives an overview of the structure of the website and the development process. It explains the core building blocks of the software and details the functionality of the algorithms. The differences to the previous versions of the tool are elaborated later on in the chapter.

The various components of the tool are the corpus, web design, algorithms, APIs, and more.


\section{Corpus}

Instead of searching webpages on the Internet the tool uses a local collection of texts in its text-search. The corpus used resembles the fictional library of ``equivalent books'' from Alfred Jarry's \emph{Exploits and Opinions of Dr.\ Faustroll, $'$Pataphysician} \autocite[p.10-12]{Jarry1996}\footnote{``In addition, three prints hanging on the walls, a poster by TOULOUSE-LAUTREC, \emph{Jane Avril}; one by BONNARD, advertising the \emph{Revue Blanche}; a portrait of Doctor Faustroll, by AUBREY BEARDSLEY\@; and an old picture, which appeared to us to be valueless, \emph{Saint Cado}, issued by the Oberthuer printing house of Rennes.''\autocite[p.12]{Jarry1996}}. In principle the corpus is just a folder within the tool's directory structure which contains the following files:
...
\lstinputlisting[firstline=62, lastline=66]{back/code/textsurfer.py}
...
stopwords see appendix~\ref{app:code}
