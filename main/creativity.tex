...
\tikzstyle{format} = [draw, thin, minimum height=2em, minimum width=2cm]
\begin{figure}[!htbp]
  \centering
  \begin{tikzpicture}[node distance=3cm, auto,>=latex', thick]
    \path[->] node[format] (minic) {mini-c};
    \path[->] node[format, below of=minic] (littlec) {little-c}
            (minic) edge node [swap] {Tinkering} (littlec);
    \path[->] node[below of=littlec, node distance=1.5cm] (reflect) {Reflection}
            (littlec) edge node {} (reflect);
    \path[->] node[format, right of=minic, node distance=7cm] (proc) {Pro-c}
                  (minic) edge node {Formal Apprenticeship} (proc)
                  (littlec) edge node [swap, text width=2cm] {Informal Apprenticeship} (proc);
    \path[->] node[above of=proc, node distance=1.5cm] (stasis) {Stasis}
            (proc) edge node {} (stasis);
    \path[->] node[format, right of=littlec, node distance=7cm] (bigc) {Big-C}
            (proc) edge node {} (bigc);
    \path[->] node[below of=bigc, node distance=1.5cm] (legend) {Legend}
            (bigc) edge node {} (legend);
    \path[->, draw] (proc) -| +(1.4,0) |- node[near start] {Greatness} (bigc);
  \end{tikzpicture}
...
\begin{quotation}
  [Creativity is] the ability to come up with ideas or artefacts that are \textbf{new, surprising and valuable}. \sourceatright{\autocite{Boden2003} (her emphasis).}
\end{quotation}
...
\begin{quotation}
  Boden argues that process does matter, stating that a program is creative only if it produces items in the right way --- by transforming the boundaries of a conceptual space. This, she claims, can only be done if the program contains reflexive descriptions which mark its own procedures and is capable of varying them. The program should contain a meta-level which assesses methods of transforming a space and considers when and how to apply them. \sourceatright{\autocite[p.8]{Pease2001}}
\end{quotation}
...
\todo{avoid value judgements, get rid of `good' below}
...
\todo{not just could be relevant - it is, refer back to hendler and hugill}
...
\todo{introduce this earlier (last setnces) - I think creative computing is interested in "tacit knowledge", which computational creativity most certainly is not.
}
...
\todo{references}
\todo{it has a journal, conferece and UG courses }
...
\todo{explain that obfuscation isnt necesarily the hallmark of creat comp}
...
\begin{quotation}
  Computational Creativity is the art, science, philosophy and engineering of computational systems which, by taking on particular responsibilities, exhibit behaviours that unbiased observers would deem to be creative. \sourceatright{(\ac{ICCC}14 website)}
\end{quotation}
...
The ACC manages the annual \ac{ICCC}, whose recent call for papers  (for \ac{ICCC} 2014) gives a useful insight into their research agenda. It can be broken down as follows:
...
The different \textbf{types} of \ac{CC} highlight the different aspects researcher and practitioners focus on during their work. These are
...
% \begin{draft}
%   Drucker's ideas on structured data, metadata and knowledge representation link very nicely into my project. How can we represent and structure data so that it does not lose its subjectivity, context and meaning? Her reference to graphical analogies is inspiring in that regard as well.
% \end{draft}
