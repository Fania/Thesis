...
There are many tools and libraries available for \ac{NLP}, including the \ac{NLTK} Python library I used for \url{pata.physics.wtf} \autocite{Bird2009, NLTK2016}.
...
\subsubsection{N-Grams \& Smoothing}
...
The basic way to compute the probability of an N-gram is shown in equation~\ref{eq:probwh}\marginnote{$\bm{\Sigma}$~\ref{eq:probwh}} \autocite{Jurafsky2009} $P(w|h)$ of a word $w$ given some history $h$ (i.e. the previous words).
...
To calculate the probability of a whole sentence of $n$ words ($P(w_1,w_2,\ldots,w_n)$ or $P(w_1^n)$ for short) we can use the chain rule of probability defined as shown in equation~\ref{eq:probw1n}\marginnote{$\bm{\Sigma}$~\ref{eq:probw1n}} \autocite{Jurafsky2009}.
...
The \ac{MLE} can be used to actually calculate these probabilities mentioned above. Counts ($C$) are normalised between 0 and 1. The \ac{MLE} in equation~\ref{eq:probwnt}\marginnote{$\bm{\Sigma}$~\ref{eq:probwnt}} \autocite{Jurafsky2009} is based on N-grams rather than bigrams.
...
These probabilities are usually generated using a training corpus. These training sets are bound to have incomplete data and certain n-grams might be missed (which will result in a probability of 0). Smoothing techniques help combat this problem. 

One example is the so-called Laplace or add-one smoothing, which basically just adds 1 to each count. See equation~\ref{eq:padd1}\marginnote{$\bm{\Sigma}$~\ref{eq:padd1}} \autocite{Jurafsky2009}.
...
More specialised sets of tags exist such as the \textit{Penn Treebank} tagset \autocite{Marcus1993} consisting of 48 different tags, including $CC$ for coordinating conjunction, $CD$ for cardinal number, $NN$ for noun singular, $NNS$ for noun plural, $NNP$ for proper noun singular, $VB$ for verb base form, $VBG$ for verb gerund, $DT$ for determiner, $JJ$ for adjectives, etc.
...
\subsubsection{Hidden Markov \& Maximum Entropy}

Hidden Markov or maximum entropy models are used for sequence classification, e.g.\ part-of-speech tagging. 
...
A classifier like the maximum entropy model will usually produce a probability of an observation belonging to a specific class. Equation~\ref{eq:pcx}\marginnote{$\bm{\Sigma}$~\ref{eq:pcx}} shows how to calculate such a probability \autocite{Jurafsky2009}.
...
\begin{conditions}
p(c|x)   & the probability of something being of class $c$ given $x$ \\
c        & the class \\
x        & the observation (i.e. the word) \\
...
Stemming techniques that are useful for this are the following \autocite[Ch.2]{Manning2009}.
