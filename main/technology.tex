...
\todo{put section into foundations chapter?}
...
Searching can be thought of in two ways, information lookup (\textbf{searching}) and exploratory search (\textbf{browsing}) \citep{DeVries1993, Marchionini2006}. A situation where an \gls{in} cannot easily be articulated or in fact is not existent (the user is not looking for anything specific) can be considered a typical case of exploratory search and describes the kind of search that is most suited to our proposed tool. The former can be understood as a type of simple question answering while the latter is a more general and broad knowledge acquisition process without a clear goal.
...
When it comes to exploratory search though, the user’s success in finding the right information depends a lot more on constraining factors such as those mentioned earlier and can sometimes benefit from a combination of information lookup and exploring \citep{Marchionini2006}.

\begin{quote}
  Much of the search time in learning search tasks is devoted to examining and comparing results and reformulat-ing queries to discover the boundaries of meaning for key concepts. Learning search tasks are best suited to combinations of browsing and analytical strategies, with lookup searches embedded to get one into the correct neighbourhood for exploratory browsing. \citep{Marchionini2006}
\end{quote}

De Vries called this form of browsing an ``enlargement of the problem space'', where the problem space refers to the resources that possibly contain the answers/solutions to the information need \citep{DeVries1993}. This is a somewhat similar idea to that of Boden’s conceptual spaces which she called the ``territory of structural possibilities'' and exploration of that space ``exploratory creativity'' \citep{Boden2003}.
...
An \gls{ir} model is a quadruple $[D,Q,F,R(q_{i}, d_{j})]$ where:\\
...
\itab{$R(q_{i}, d_{j})$}
            \tab{is the ranking function, where $q_{i} \in Q$ and $d_{j} \in D$,}\\
...
% An \gls{ir} model is a quadruple $[D,Q,F,R(q_{i}, d_{j})]$ where:
% \begin{description}
%   \item [$D$] is the set of documents
%   \item [$Q$] is the set of queries
%   \item [$F$] is the framework (e.g. sets, Boolean relations, vectors, linear algebra\ldots)
%   \item [$R(q_{i}, d_{j})$] is the ranking function, where $q_{i} \in Q$ and $d_{j} \in D$
%   \item [$t$] is the number of index terms in document collection
%   \item [$V =\{k_{1},\ldots, k_{t}\}$] is the set of all distinct index terms in document collection (vocabulary)
% \end{description}
...
\todo{describe index}
An index or inverted index is blah blah blah....
diagram?
...
\begin{equation}
  ...
  \textrm{max}(i,j) & \textrm{if min}(i,j)=0\\
  \textrm{min}\left\{\begin{matrix*}[l]
  d_{a,b}(i-1,j)+1\\
  d_{a,b}(i,j-1)+1\\
...
\gls{nlp} blah blah blah...

Bird, S., Klein, E. and Loper, E., 2009. \gls{nlp} with Python 1st ed., Sebasopol, CA: O'Reilly Media.\citep{Bird2009}
