...
Regular expressions (often shortened to the term `regex') are used to search a corpus of texts for the occurance of a specific string pattern\footnote{Or you can just use it to play Regex Crosswords online: \url{http://regexcrossword.com/}.}. 
...
\paragraph{Chain rule of probability}
...
\paragraph{Markov assumption} that probability of a word depends only on the previous word (or $n$ words).
...
\paragraph{maximum likelihood estimation (MLE)} for $N$-Grams we can normalise counts to be between 0 and 1. $C$ stands for count.
...
\subsubsection{Markov Chains \& Maximum Entropy}
