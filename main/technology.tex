...
where $c(x)$ is the `conjunctive component' of $x$. A conjunctive component describes which terms occur in a document and which ones do not. E.g.\ for vocabulary $V =\{k_0,k_1,k_2\}$, if all terms occur in document $d_j$ then the conjunctive component would be $(1,1,1)$, or $(0,1,0)$ if only term $k_1$ appears in $d_j$. Let's make this clearer with a practical example.

Figure~\ref{fig:termdocs} shows a vocabulary of 9 terms over 3 documents. 

\begin{conditions}
V & $\{Faustroll, father, time, background, water, doctor, without, bishop, God\}$ \\
d_0 & Faustroll \\
d_1 & Gospel \\
d_2 & Voyage \\
c(d_0) & $(1,1,1,0,1,1,1,1,1)$ \\
c(d_1) & $(0,1,1,0,1,0,1,0,1)$ \\
c(d_2) & $(0,1,1,0,1,0,1,1,1)$ \\
q & $[doctor \land (Faustroll \lor \neg God)]$ \\
...
Ranking is the process of ordering search results using a given weight. One simple method of ranking is the so-called \acl{TF}-\acl{IDF} or \acs{TF}-\acs{IDF} for short. Given a \acs{TF} weight of $tf_{i,j}$ and a \acs{IDF} weight of $idf_j$ it is defined as $tf_{i,j}\times idf_j$.
...
The \ac{IDF} $idf_j$ weight is calculated as $\log \frac{N}{df_i}$, where the document frequency $df_i$ is the number of documents in a collection that contain a term $k_i$ and $idf_i$ is the \ac{IDF} of term $k_i$. The more often a term occurs in different documents the lower the \ac{IDF}.
