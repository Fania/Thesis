...
Ranking is the process of ordering search results using a given weight. One simple method of ranking is the so-called \acrlong{tf}-\acrlong{idf} weighting or \acrshort{tf}-\acrshort{idf} weighting for short. Given a \gls{tf} weight of $tf_{i,j}$ and a \gls{idf} weight of $idf_j$ it is defined as $tf_{i,j}\times idf_j$.
...
Where $w_{i,j}$ is the weight associated with $(k_i,d_j)$. Using this formula enusres that rare terms have a higher weight and more so if they occur a lot in one document.
...
The total \gls{tf} $F_i$ is calculated as $\sum_{j=1}^{N}f_{i,j}$, where $F_i$ is the total frequency of term $k_i$ in the collection and $f_{i,j}$ is the frequency of occurance of term $k_i$ in document $d_j$ and $N$ is the total number of documents.
...
The notation for \gls{ir} models is as follows \citep[adapted from][p.58]{Baeza-Yates2011}:
...
\todo{change font again because of ligatures?}
...
One such ranking score is the Boolean model. The similarity of document $d_j$ to query $q$ is defined as follows (quoted from \citep[p.65]{Baeza-Yates2011})
...
\hypertarget{stemming}{Techniques} that are useful for this are the following \citep[Ch.2]{Manning2009}.
