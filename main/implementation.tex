...
The website \url{http://pata.physics.wtf} (see image~\ref{img:fullscreenshot}\marginpar{\faicon{picture-o}~\ref{img:fullscreenshot}}) embodies the knowledge of this doctoral research and showcases \acf{AMC} and patalgorithms. This chapter gives an overview of the structure of the website and the development process.
...
The front-end design uses the \textit{W3.CSS} stylesheet \autocite{w3css} as a basis. The website is mostly responsive (see image~\ref{img:proto3screen}\marginpar{\faicon{picture-o}~\ref{img:proto3screen}}), meaning it can be viewed well on phones, tablets and desktop screens (the poems and image spirals for example unfortunately have a fixed width which does not scale down well). The site contains various scripts written in \textbf{JavaScript} (e.g. scramble letters, randomise poem, send email and tabbed content).
...
The folder structure is shown in figure~\ref{fig:files}\marginpar{\faicon{object-group}~\ref{fig:files}}. Each spot represents one file.
...
Putting it another way, (1) the system setup tokenises each of the source texts, removes stopwords and then adds terms and their location to the index (see section~\ref{s:index}\marginpar{§~\ref{s:index}}), (2) a query then triggers the three pataphysical algorithms, (3) each algorithm finds results for the query (see section~\ref{s:algorithms}\marginpar{§~\ref{s:algorithms}}), (4) some words before/after the match are retrieved for context, and (5) the resulting sentences are rendered for the user.
...
Each plain text file in the corpus is added to the internal library one by one. Source~\ref{code:addtocorpus}\marginpar{\faicon{code}~\ref{code:addtocorpus}} shows how this is done. The \py{PlaintextCorpusReader} is a feature of the \ac{NLTK} Python library \autocite{NLTK2016} for \acf{NLP}. The \py{words} function tokenises the text, i.e. it splits it into individual words and stores them as an ordered list.
...
The \py{setupcorpus} function (see source~\ref{code:setupcorpus}\marginpar{\faicon{code}~\ref{code:setupcorpus}}) is called for each of the text files in the two corpora to populate the index data structures \py{l_dict} (for the Faustroll vocabulary) and \py{s_dict} (for the Shakespeare vocabulary).

\begin{minted}{text}
dict = dictionary { dictionary { list [ ] } }
\end{minted}

A dictionary in Python is what is known as an `associative array' in other languages. Essentially they are unordered sets of \emph{key: value} pairs. The \py{dict} used here is a dictionary where each key has another dictionary as its value. Each nested dictionary has a list as the value for each key.
...
Line 7 in source~\ref{code:setupcorpus}\marginpar{\faicon{code}~\ref{code:setupcorpus}} starts looping through file \py{f}. Line 8 checks if the current word \py{w} contains anything other than alphabetical characters and whether or not \py{w} is contained in the relevant stop-word file \py{lang} (for a list of English stopwords see appendix~\ref{s:stopwords}\marginpar{§~\ref{s:stopwords}}). If both of those conditions are true, a variable \py{y} is created on line 9 (such as `l\_00' based on `00.faustroll.txt') and \py{w} is added to the relevant dictionary file \py{dic} together with \py{y} and the current position \py{x} on line 10. After all files are processed, the two index structures look roughly like this:
...
Figure~\ref{fig:textsearch}\marginpar{\faicon{object-group}~\ref{fig:textsearch}} previously showed the rough sequence of events in text search and highlighted that the pataphysicalisation from query to patadata happens in the \py{textsurfer.py} Python script file.


\subsection{Clinamen}
\label{s:clinamenalgo}

The clinamen was introduced in chapter~\ref{s:clinamen}\marginpar{§~\ref{s:clinamen}} but to briefly summarise it, it is the unpredictable swerve that Bök calls ``the smallest possible aberration that can make the greatest possible difference'' \autocite*{Bok2002}.
...
Source~\ref{code:clinamen}\marginpar{\faicon{code}~\ref{code:clinamen}} line 6 creates the set of clinamen words using a list comprehension. It retrieves matches from the Faustroll file \py{l_00} with the condition that they are of Damerau-Levenshtein distance \py{i} or less to the query term \py{w} (see source~\ref{code:dl}\marginpar{\faicon{code}~\ref{code:dl}}). Duplicates are removed. Line 7 then makes a call to the generic \py{get_results} function\marginpar{\faicon{code}~\ref{code:getresults}} to get all relevant result sentences, the list of source files and the total number of results.
...
The \py{get_results} function (see source~\ref{code:getresults}\marginpar{\faicon{code}~\ref{code:getresults}}) is used by all three text algorithms (clinamen, syzygy and antinomy). Given the nested structure of the indexes \py{l_dict} and \py{s_dict}, the function loops through each of the \py{words} passed to it (\py{r}) first and then each file in \py{files.items()}. Lines 8 and 9 retrieve the dictionary of files for term \py{r} from the relevant dictionary. Line 13 gets the author and full title of file \py{e} and adds it to the list of sources in line 14. Line 15 makes use of another function called \py{pp_sent} (see source~\ref{code:ppsent}\marginpar{\faicon{code}~\ref{code:ppsent}}) to get an actual sentence fragment for the current word \py{r} in file \py{e}, which is then added to the output. The output is structured as a triple containing the author and title, the list of resulting sentences and the name of the algorithm used.
...
In function \py{pp_sent} (source~\ref{code:ppsent}\marginpar{\faicon{code}~\ref{code:ppsent}}) line 5 is important to note because it is a key functionality point. Even though the index files store a full list of all possible positions of a given word in each file, the \py{pp_sent} function only retrieves the sentence of the very first occurrence of the word rather than each one. This decision was taken to avoid overcrowding of results for the same keyword and is further discussed in chapter~\ref{s:sents}\marginpar{§~\ref{s:sents}}.
...
The syzygy function makes heavy use of WordNet \autocite{Miller1995} through the \ac{NLTK} Python library \autocite{NLTK2016} to find suitable results (importing it using the following command \py{from nltk.corpus import wordnet as wn}). Specifically, as shown in source~\ref{code:syzygy}\marginpar{\faicon{code}~\ref{code:syzygy}}, the algorithm fetches the set of synonyms (synsets) on line 5. It then loops through all individual items \py{ws} in the list of synonyms \py{wordsets} in line 7--20. It finds any hyponyms, hypernyms, holonyms, and meronyms for \py{ws} (each of which denotes some sort of relationship\marginpar{\faicon{object-group}~\ref{fig:wordnet}} or membership with its parent synonym---see figure~\ref{fig:wordnet}) using the \py{get_nym}\marginpar{\faicon{code}~\ref{code:getnym}} function (see lines 8, 11, 14, and 17). Line 21 makes use of the \py{get_results} function (see source~\ref{code:getresults}\marginpar{\faicon{code}~\ref{code:getresults}}) in the same way as the clinamen function does.
...
The antinomy, in a pataphysical sense, is the mutually incompatible. It was previously introduced in chapter~\ref{s:antinomy}\marginpar{§~\ref{s:antinomy}}. In simple terms, the antinomy algorithm works in two steps:
...
For the antinomy\marginpar{\faicon{code}~\ref{code:antinomy}} I simply used WordNet's antonyms (opposites) (source~\ref{code:antinomy}). In principle, this function is similar to the algorithm for the syzygy. It finds all antonyms through \ac{NLTK}'s \py{lemmas()[0].antonyms()} function on line 7 and retrieves result sentences using the \py{get_results} function\marginpar{\faicon{code}~\ref{code:getresults}} on line 12.
...
A formal description of the \url{pata.physics.wtf} system in terms of an \ac{IR} model described in chapter~\ref{s:irmodels}\marginpar{§~\ref{s:irmodels}} is unsuitable. It assumes for example the presence of some sort of ranking algorithm $R(q_i, d_j)$.
...
\py{damerauleveshtein(q,p)} in equation~\ref{eq:clinamen}\marginpar{$\bm{\Sigma}$~\ref{eq:clinamen}} is the Damerau-Levenshtein algorithm as described in section~\ref{code:dl}\marginpar{\faicon{code}~\ref{code:dl}} and $v_0$ is the Faustroll text.
...
\py{synonyms(q)} in equation~\ref{eq:syzygy}\marginpar{$\bm{\Sigma}$~\ref{eq:syzygy}} is the WordNet/\ac{NLTK} function to retrieve all synsets for the query $q$ and the four `nym' functions return the relevant hyponyms, hypernyms, holonyms or meronyms for each of the synonyms.
...
Similarly, in equation~\ref{eq:antinomy}\marginpar{$\bm{\Sigma}$~\ref{eq:antinomy}} the \py{synonyms(q)} function returns WordNet synsets for the query $q$ and the \py{antonyms(s)} function returns WordNet antonyms for each of the synonyms.
...
The set of results $R(P(q))$ can then be defined as shown in equation~\ref{eq:results}\marginpar{$\bm{\Sigma}$~\ref{eq:results}}. It returns a list of triples containing the source text $d$, the sentence \py{sent(p)} and the algorithm $f$. For each pataphysicalised query term $p$ one sentence is retrieved per file $d$.


\section{Image \& Video}
\label{s:imgvid}

The image and video search of \url{pata.physics.wtf} both work slightly differently to the text search described in section~\ref{s:algorithms}\marginpar{§~\ref{s:algorithms}}. In simple terms, the image and video search works in three steps:
...
The next step is to pataphysicalise the translated query (see source~\ref{code:pataph}\marginpar{\faicon{code}~\ref{code:pataph}}). The \py{pataphysicalise} function transforms this translation in a process slighlty simplified from the \py{syzygy} algorithm\marginpar{\faicon{code}~\ref{code:syzygy}}. The decision to simplify the algorithm was made due to performance issues related to the \ac{API} calls that follow in the final step of the search process. 
...
Source~\ref{code:getFlickr}\marginpar{\faicon{code}~\ref{code:getFlickr}} shows how such an \ac{API} call is made using JavaScript (in this case Flickr). Source~\ref{code:imglist}\marginpar{\faicon{code}~\ref{code:imglist}} below shows how 10 seperate images are collected into one results list and the \py{createSpiral} function is called to render the images to the user in \ac{HTML} (see appendix~\ref{s:imgspiral}\marginpar{§~\ref{s:imgspiral}} for the relevant code snippet).
...
The Bing and Getty searches work in a similar way with one exception. Getty does not populate the output list by doing 10 individual \ac{API} calls but rather by adding 10 results from 1 call. This is due to a time restriction in the Getty \ac{API}; it does not allow 10 calls in a second.
...
Once the \py{imglist} contains 10 items it is passed to the \py{createSpiral} function which renders it to \ac{HTML}. Appendix~\ref{s:bingjson}\marginpar{§~\ref{s:bingjson}} shows an example shortnened \ac{JSON} result from Bing.

\spirals

The video search also uses an \ac{API} to retrieve results. This function is written in Python and uses the \textit{Requests} library \autocite{Reitznd} to make the \ac{API} calls to YouTube \autocite{YouTubeAPI} as shown in source~\ref{code:videosearch}\marginpar{\faicon{code}~\ref{code:videosearch}}.

First, the query is translated using the \py{transent} function\marginpar{\faicon{code}~\ref{code:transent}} on line 3. Line 4 separates the English translation into its own list \py{transplit} which is then pataphysicalised on line 5 using the algorithm described in source~\ref{code:pataph}\marginpar{\faicon{code}~\ref{code:pataph}}.
...
The video results are then also displayed in a golden spiral in the same way as the images. This is described in section~\ref{s:spiral}\marginpar{§~\ref{s:spiral}}.
...
One of the key requirements for the \textit{Syzygy Surfer} tool\marginpar{§~\ref{s:surfer}} was that ``the user should be able to choose the techniques they use'' \autocite{Hendler2011}. This has been adopted for \url{pata.physics.wtf} in the sense that the user has different options for the display of results.
...
The overal visual design is shown in image~\ref{img:proto3screen}\marginpar{\faicon{picture-o}~\ref{img:proto3screen}}.


\subsection{Poetry}
\label{s:poetry}

Source~\ref{code:qpoems}\marginpar{\faicon{code}~\ref{code:qpoems}} shows the segment of \ac{HTML}/Jinja code that renders the Queneau poetry. The code renders the 4 stanzas of the poem. This is done using two nested Jinja \py{for} loops (line 2 and line 10). Line 2 loops through the (ideally) 14 lines of the poem. \py{lol} can be considered a masterlist of all sublists for each poem line.
...
\py{all_sens} is the pool of all sentences. It is structured as follows.

\begin{minted}{text}
[(title, (pre, word, post), algorithm), ...]
\end{minted}

\py{lol} is a list subdivided into partitions for each line of the sonnet. Let's say there are 350 sentences overall in \py{all_sens}. To divide them equally among the 14 lines of a sonnet, we need to create \py{lol} with 14 equal parts of 25 sentences.
...
Changing a line of the poem is achieved by clicking on one of the buttons on either side of the poem's line (as shown in image~\ref{img:qpoemtree}\marginpar{\faicon{picture-o}~\ref{img:qpoemtree}}). This will trigger a JavaScript function (based on \autocite{DYNWEB2016}) to automatically scroll to the next sentence. 
...
The two other ways to display text results are as a list ordered by source or by patalgorithm which works in a similar way to what is described in source~\ref{code:textlists}\marginpar{\faicon{code}~\ref{code:textlists}}. The code is wrapped in an \ac{HTML} unordered list tag \py{<ul>}. A Jinja \py{for} loop generates the individual \py{<li>} tags on line 4.
...
Image~\ref{img:listsourcetree}\marginpar{\faicon{picture-o}~\ref{img:listsourcetree}} shows a shortened example set of results for query `tree' ordered by source, that is, ordered by original file.
...
Figure~\ref{img:listalgotree}\marginpar{\faicon{picture-o}~\ref{img:listalgotree}} shows a shortened example set of results for query `tree' ordered by patalgorithm, that is, ordered by the algorithm which produced the patadata.
...
The image and video spirals are constructed in complicated nested \ac{HTML} components. The code for generating an image spiral is shown in appendix~\ref{s:imgspiral}\marginpar{§~\ref{s:imgspiral}}. The video spiral is constructed in a similar way but directly in the \ac{HTML} file as opposed to in the JavaScript file. The video spiral is almost identical, the only difference is the biggest 5 videos are actually embedded as videos. The smaller 5 videos are shown as still images which link to the relevant YouTube page.

Generally, the idea was taken from the pataphysical \emph{grand gidouille} (see chap\-ter~\ref{ch:pataphysics}\marginpar{§~\ref{ch:pataphysics}}) and represented as a Fibonacci spiral. Figure~\ref{img:fibspiral}\marginpar{\faicon{object-group}~\ref{img:fibspiral}} shows a spiral created using the Flickr image search for query `blue mountains' overlaid with a white Fibonacci spiral to highlight the structure.
...
The final website \url{pata.physics.wtf} went through several iterations of development since it was first conceived in 2012. This included 3 major technical updates since the first prototype and 2 new visual re-designs. Table~\ref{tab:versions}\marginpar{\faicon{table}~\ref{tab:versions}} shows the main differences and similarities between the versions. 
...
\vspace*{-0.2cm}
Many of the influences (§~\ref{ch:inspirations} \inspi) and pataphysical inspirations (§~\ref{ch:pataphysics} \pata) are visited again here. Necessary technical background for topics such as the index data structure are given in chapter~\ref{ch:technology} \tech. The actual pataphysicalisation process and the algorithms which are part of it are described in section~\ref{s:algorithms} \imple, while the design process is explained in section~\ref{s:design} \imple. Both of these topics are revisited in the \nameref{ch:analysis} \anal~ chapter, including looking at numbers of results (§~\ref{s:numbers} \anal), and the index and corpora (§~\ref{s:analindex} \anal). Potential future work is highlighted in chapter~\ref{ch:future} \aspi, such as performance improvements (§~\ref{s:performance}), design changes (§~\ref{s:designaspi}), adaptations or additions to the text search process (§~\ref{s:textfuture} and \ref{s:pataasp}), and the missing link to audio search (§~\ref{s:audio}).
