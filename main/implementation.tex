...
\todo{add top level description of the site first}
repeat arty bollocks from intro 
add arty bollocks to abstract

a pataphysical search engine
subverts conventional expectations
recombining source texts into emergent user directed poetical structures
ephemeral connections made concrete by emails
unpredictable spirals of pataphysicalised visual Media
illustrate AMC
...
\begin{figure}[!htbp] % (here, top, bottom, page)
  ...
  \begin{tikzpicture}
    ...
    \fill (3,5) circle (2pt) node (corpus) {};
...
Each plain text file in the corpus is added to the internal library one by one. Source~\ref{code:addtocorpus} shows how this is done. The \py{PlaintextCorpusReader} is a feature of the \ac{NLTK} Python library\footnote{\url{http://www.nltk.org/}} for \acl{nlp}.
...
Line \num{6} in source~\ref{code:setupcorpus} starts looping through file \py{f}. Line \num{7} checks if the current word \py{w} contains anything other than alphabetical characters and whether or not \py{w} is contained in the relevant stopword file \py{l} (for a list of english stopwords see appendix~\ref{app:code}). If both of those conditions are true variable \py{y} is created on line \num{8} (such as `l\_00' based on `00.faustroll.txt') and \py{w} is added to \py{l_dict} together with the file \py{y} and the current position \py{x} on line 9. After all files are processed, the index looks like this:
